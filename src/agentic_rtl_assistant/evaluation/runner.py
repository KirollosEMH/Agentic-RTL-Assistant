...
class EvaluationRunner:
    ...
    async def run(self) -> Path:
        ...
        metrics = {
            ...
            "input_tokens": sum(
                int(result["usage"]["input_tokens"]) for result in results
            ),
            "output_tokens": sum(
                int(result["usage"]["output_tokens"]) for result in results
            ),
        }
        ...
        traces = "\n".join(
            json.dumps(event.as_dict()) for event in service.telemetry.events
        )
